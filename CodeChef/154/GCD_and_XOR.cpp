...
inline void solve()
{
    ...
    set<int> s;
    int g=0;
    for(int i:ar) {g=gcd(i,g); s.insert(i);}
    
    int range=0;
    for(int i=0;i<n;i++){
        if(ar[i]==k) {continue;}
        if(i==0 || ar[i-1]==k) range++;
    }

    s.erase(k);
    //if all elements are divisible by k
    // then the GCD will also divisible by k
    if(s.size()==0) {cout<<0<<endl;return;}
    if(g%k==0 || (*s.begin()==*s.rbegin()) && range==1) cout<<1<<endl;
    else cout<<2<<endl;
}
int32_t main()
{
