...
inline void solve()
{
    ...
    int maxBob = max(vec[0].ff,vec[0].ss);
    bool flag=false;
    for(int i=0;i<n;i++){
        if(ar1[i]==vec[0].ff){continue;}
        int cur_mx = max(ar1[i],ar2[i]);
        if(cur_mx>maxBob){flag=true;break;}
    }
    
    if(!flag && n>1){
        int maxBob = max(vec[1].ff,vec[1].ss);
        for(int i=0;i<n;i++){
            if(ar1[i]==vec[1].ff){continue;}
            int cur_mx = max(ar1[i],ar2[i]);
            if(cur_mx>maxBob){flag=true;break;}
        }
    }
